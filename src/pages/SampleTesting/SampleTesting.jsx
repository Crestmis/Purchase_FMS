...
import { ClipboardList, Clock, Beaker, Search, Filter, ChevronDown, ArrowUp, ArrowDown } from 'lucide-react';
import PendingSampleTesting from './PendingSampleTesting';
import HistorySampleTesting from './HistorySampleTesting';
...
const SampleTesting = () => {
    ...
    useEffect(() => {
        // Mock data initialization for the Sample Testing Flow
        const initializeSampleTesting = () => {
            const hasInitialized = localStorage.getItem('crest_purchase_testing_initialized');

            if (!hasInitialized) {
                // Generate 5 pending items (mocking verified docs arriving here)
                ...
                const history = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    quotedPrice: (i + 1) * 950,
                    leadTime: (i % 3) + 1,
                    vehicleNo: `MH 12 CD ${6000 + i}`,
                    invoiceNo: `INV-24-${3000 + i}`,
                    status: i % 4 === 0 ? 'Fail' : 'Pass',
                    remarks: i % 4 === 0 ? 'Purity below threshold' : 'All parameters within range'
                }));

                localStorage.setItem('crest_purchase_pending_testing', JSON.stringify(pending));
                localStorage.setItem('crest_purchase_history_testing', JSON.stringify(history));
                localStorage.setItem('crest_purchase_testing_initialized', 'true');

                setPendingIndents(pending);
                setHistoryIndents(history);
            } else {
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_testing') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_testing') || '[]'));
            }
        };
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_testing', JSON.stringify(updatedPending));
        localStorage.setItem('crest_purchase_history_testing', JSON.stringify(updatedHistory));
    };
    ...
    return (
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-sky-100 shadow-sm">
                <div className="flex-1">
                    <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                        <Beaker className="text-sky-500" />
                        Sample Testing
                    </h1>
                    <p className="text-sm text-slate-500 mt-1">Quality check and lab testing of incoming material samples</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="relative shrink-0">
                        ...
                        <Beaker className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-500" size={14} />
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingSampleTesting indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                ) : (
                    <HistorySampleTesting indents={filteredHistory} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                )}
            </div>
        </div>
    );
