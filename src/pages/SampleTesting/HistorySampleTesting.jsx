...
const HistorySampleTesting = ({ indents, SortDropdown, onSort }) => {
    return (
        <div className="flex-1 bg-white/90 backdrop-blur-sm border border-sky-100 rounded-2xl shadow-lg overflow-hidden flex flex-col">
                        ...
                        <tr>
                            ...
                            >
                                <div className="flex items-center">
                                    Test Result <SortDropdown columnKey="status" />
                                </div>
                            </th>
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                        ...
                        {indents.map((item) => (
                            <tr key={item.id} className="hover:bg-sky-50/50 transition-colors">
                                <td className="px-4 py-3">
                                    ...
                                    `}>
                                        {item.status === 'Pass' ? <CheckCircle size={12} /> : <XCircle size={12} />}
                                        {item.status}
                                    </span>
                                </td>
            ...
            <div className="lg:hidden flex-1 overflow-y-auto p-4 space-y-4">
                {indents.map((item) => (
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                        <div className="flex justify-between items-start border-b border-slate-50 pb-3">
                            <div>
                                <div className="flex items-center gap-2">
                                    ...
                                    `}>
                                        {item.status === 'Pass' ? <CheckCircle size={10} /> : <XCircle size={10} />}
                                        {item.status}
                                    </span>
                                </div>
                        ...
                        <div className="bg-slate-50/50 p-2.5 rounded-xl border border-slate-100 italic text-xs text-slate-600">
                            <label className="text-[10px] font-bold text-slate-400 uppercase block not-italic mb-1">Remarks</label>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div className="bg-purple-50/50 p-2.5 rounded-xl">
            ...
            {indents.length === 0 && (
                <div className="text-center py-12 text-slate-400">
                    ...
                    <p>No sample testing history found.</p>
                </div>
            )}
        </div>
    );
...
export default HistorySampleTesting;
