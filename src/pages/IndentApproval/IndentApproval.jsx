...
import { ClipboardList, Clock, CheckSquare, Search, Filter, Box, ChevronDown, ArrowUp, ArrowDown } from 'lucide-react';
...
const IndentApproval = () => {
    ...
    useEffect(() => {
        // Mock data initialization for the Approval Flow
        const initializeApprovals = () => {
            const hasInitialized = localStorage.getItem('crest_purchase_approval_initialized');
            
            if (!hasInitialized) {
                // Generate 5 pending items
                ...
                const history = Array.from({ length: 10 }).map((_, i) => ({
                    ...
                    closingStock: (i + 1) * 10,
                    reorderQty: (i + 1) * 30,
                    status: i % 3 === 0 ? 'Reject' : 'Approve' // Mix of Approve and Reject
                }));

                localStorage.setItem('crest_purchase_pending_approvals', JSON.stringify(pending));
                localStorage.setItem('crest_purchase_history_approvals', JSON.stringify(history));
                localStorage.setItem('crest_purchase_approval_initialized', 'true');
                
                setPendingIndents(pending);
                setHistoryIndents(history);
            } else {
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_approvals') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_approvals') || '[]'));
            }
        };
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_approvals', JSON.stringify(updatedPending));
        localStorage.setItem('crest_purchase_history_approvals', JSON.stringify(updatedHistory));
    };
        ...
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-sky-100 shadow-sm">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500 mt-1">Review, approve, or reject material indents</p>
                </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingApproval indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                ) : (
                    <HistoryApproval indents={filteredHistory} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                )}
            </div>
        </div>
    );
