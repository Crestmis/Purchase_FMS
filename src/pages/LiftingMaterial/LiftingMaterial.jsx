...
import { ClipboardList, Clock, Truck, Search, Filter, ChevronDown, ArrowUp, ArrowDown } from 'lucide-react';
import PendingLiftingMaterial from './PendingLiftingMaterial';
import HistoryLiftingMaterial from './HistoryLiftingMaterial';

const LiftingMaterial = () => {
    ...
    useEffect(() => {
        // Mock data initialization for the Lifting Material Flow
        const initializeLiftingMaterial = () => {
            const hasInitialized = localStorage.getItem('crest_purchase_lifting_initialized');

            if (!hasInitialized) {
                // Generate 5 pending items (mocking vendor confirmed POs arriving here)
                ...
                const history = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    leadTime: (i % 3) + 1,
                    status: 'Placed',
                    vehicleNo: `MH 12 AB ${1000 + i}`,
                    placementDate: new Date(Date.now() - (i * 86400000)).toISOString()
                }));

                localStorage.setItem('crest_purchase_pending_lifting', JSON.stringify(pending));
                localStorage.setItem('crest_purchase_history_lifting', JSON.stringify(history));
                localStorage.setItem('crest_purchase_lifting_initialized', 'true');

                setPendingIndents(pending);
                setHistoryIndents(history);
            } else {
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_lifting') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_lifting') || '[]'));
            }
        };

        initializeLiftingMaterial();
    }, []);
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_lifting', JSON.stringify(updatedPending));
        localStorage.setItem('crest_purchase_history_lifting', JSON.stringify(updatedHistory));
    };
        ...
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-sky-100 shadow-sm">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500 mt-1">Assign and place vehicles for dispatching PO materials</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="relative shrink-0">
                        ...
                        <Truck className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-500" size={14} />
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingLiftingMaterial indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                ) : (
                    <HistoryLiftingMaterial indents={filteredHistory} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                )}
            </div>
        </div>
    );
...
export default LiftingMaterial;
