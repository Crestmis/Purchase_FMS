...
import { ClipboardList, Clock, ShieldCheck, Search, Filter, ChevronDown, ArrowUp, ArrowDown } from 'lucide-react';
import PendingQuotationApproval from './PendingQuotationApproval';
import HistoryQuotationApproval from './HistoryQuotationApproval';
...
const QuotationApproval = () => {
    ...
    useEffect(() => {
        // Mock data initialization for the Quotation Approval Flow
        const initializeQuotationApprovals = () => {
            const hasInitialized = localStorage.getItem('crest_purchase_quotation_approval_initialized');
            
            if (!hasInitialized) {
                // Generate 5 pending items (mocking processed quotations arriving here)
                const pending = Array.from({ length: 5 }).map((_, i) => ({
                    id: Date.now() + i,
                    serialNo: `SN-P${String(i + 1).padStart(3, '0')}`,
                    sku: `SKU${String(400 + i).padStart(3, '0')}`,
                ...
                const history = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    sku: `SKU${String(500 + i).padStart(3, '0')}`,
                    ...
                    closingStock: (i + 1) * 10,
                    reorderQty: (i + 1) * 30,
                    status: i % 2 === 0 ? 'Approve' : 'Reject',
                    vendorName: `Supplier ${i + 1} Pvt Ltd`,
                    quotedPrice: (i + 1) * 1200,
                    leadTime: (i % 3) + 1,
                    remarks: i % 2 !== 0 ? 'Price too high, rejecting.' : 'Approved quote.'
                }));

                localStorage.setItem('crest_purchase_pending_q_approvals', JSON.stringify(pending));
                localStorage.setItem('crest_purchase_history_q_approvals', JSON.stringify(history));
                localStorage.setItem('crest_purchase_quotation_approval_initialized', 'true');
                
                setPendingIndents(pending);
                setHistoryIndents(history);
            } else {
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_q_approvals') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_q_approvals') || '[]'));
            }
        };

        initializeQuotationApprovals();
    }, []);
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_q_approvals', JSON.stringify(updatedPending));
        localStorage.setItem('crest_purchase_history_q_approvals', JSON.stringify(updatedHistory));
    };
        ...
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-sky-100 shadow-sm">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500 mt-1">Review and approve vendor quotations to proceed with PO</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="relative shrink-0">
                        ...
                        <ShieldCheck className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-500" size={14} />
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingQuotationApproval indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                ) : (
                    <HistoryQuotationApproval indents={filteredHistory} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                )}
            </div>
        </div>
    );
