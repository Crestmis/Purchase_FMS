...
import QuotationApprovalForm from './QuotationApprovalForm';

const PendingQuotationApproval = ({ indents, onProcess, SortDropdown, onSort }) => {
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div className="bg-slate-50/50 p-2.5 rounded-xl">
                                ...
                                <span className="text-slate-700 font-bold">{item.leadTime ? `${item.leadTime} days` : '-'}</span>
                            </div>
                            <div className="bg-slate-50/50 p-2.5 rounded-xl col-span-2">
                                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Total Requirement</label>
                        ...
                        <button 
                            onClick={() => setSelectedIndent(item)}
                            className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-xl text-xs font-bold shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all"
                        >
                            <ShieldCheck size={16} />
...
export default PendingQuotationApproval;
