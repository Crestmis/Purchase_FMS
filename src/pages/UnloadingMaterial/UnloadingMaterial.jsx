...
import { ClipboardList, Clock, PackageSearch, Search, Filter, ChevronDown, ArrowUp, ArrowDown } from 'lucide-react';
import PendingUnloading from './PendingUnloading';
import HistoryUnloading from './HistoryUnloading';
...
const UnloadingMaterial = () => {
    ...
    useEffect(() => {
        // Mock data initialization for the Unloading Material Flow
        const initializeUnloading = () => {
            const hasInitialized = localStorage.getItem('crest_purchase_unloading_initialized');

            if (!hasInitialized) {
                // Generate 5 pending items (mocking tested samples arriving here)
                const pending = Array.from({ length: 5 }).map((_, i) => ({
                    id: Date.now() + i,
                    serialNo: `SN-P${String(i + 1).padStart(3, '0')}`,
                    sku: `SKU${String(1300 + i).padStart(3, '0')}`,
                ...
                const history = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    sku: `SKU${String(1400 + i).padStart(3, '0')}`,
                    ...
                    quotedPrice: (i + 1) * 950,
                    leadTime: (i % 3) + 1,
                    vehicleNo: `MH 12 CD ${7000 + i}`,
                    invoiceNo: `INV-24-${5000 + i}`,
                    status: 'Unloaded',
                    storageLocation: `Section B - Tank ${i + 1}`,
                    unloadedQty: (i + 1) * 30
                }));

                localStorage.setItem('crest_purchase_pending_unloading', JSON.stringify(pending));
                localStorage.setItem('crest_purchase_history_unloading', JSON.stringify(history));
                localStorage.setItem('crest_purchase_unloading_initialized', 'true');

                setPendingIndents(pending);
                setHistoryIndents(history);
            } else {
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_unloading') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_unloading') || '[]'));
            }
        };
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_unloading', JSON.stringify(updatedPending));
        localStorage.setItem('crest_purchase_history_unloading', JSON.stringify(updatedHistory));
    };
    ...
    return (
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-sky-100 shadow-sm">
                <div className="flex-1">
                    <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                        <PackageSearch className="text-sky-500" />
                        Unloading of Material
                    </h1>
                    <p className="text-sm text-slate-500 mt-1">Manage physical unloading and storage assignment of tested materials</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="relative shrink-0">
                        ...
                        <PackageSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-500" size={14} />
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingUnloading indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                ) : (
                    <HistoryUnloading indents={filteredHistory} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                )}
            </div>
        </div>
    );
