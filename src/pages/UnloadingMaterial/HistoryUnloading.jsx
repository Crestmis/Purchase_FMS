...
const HistoryUnloading = ({ indents, SortDropdown, onSort }) => {
    return (
        <div className="flex-1 bg-white/90 backdrop-blur-sm border border-sky-100 rounded-2xl shadow-lg overflow-hidden flex flex-col">
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-sky-50 text-sky-700 font-medium border-b border-sky-100 sticky top-0 z-10">
                        <tr>
                            ...
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                                onClick={() => onSort("storageLocation")}
                            >
                                <div className="flex items-center">
                                    Storage Location <SortDropdown columnKey="storageLocation" />
                                </div>
                            </th>
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                    ...
                    <tbody className="divide-y divide-sky-50">
                        {indents.map((item) => (
                            <tr key={item.id} className="hover:bg-sky-50/50 transition-colors">
                                ...
                                <td className="px-4 py-3 font-bold text-sky-700 bg-sky-50/50 uppercase">
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="bg-sky-50/50 p-2.5 rounded-xl border border-sky-100">
                                <label className="text-[10px] font-bold text-sky-400 uppercase block mb-1">Storage Location</label>
                                <span className="text-sky-700 font-bold uppercase text-lg">{item.storageLocation || '-'}</span>
                            </div>
            ...
            {indents.length === 0 && (
                <div className="text-center py-12 text-slate-400">
                    ...
                    <p>No unloading history found.</p>
                </div>
            )}
        </div>
    );
...
export default HistoryUnloading;
