...
import { ClipboardList, Clock, FileSignature, Search, Filter, ChevronDown, ArrowUp, ArrowDown } from 'lucide-react';
import PendingVendorDetails from './PendingVendorDetails';
import HistoryVendorDetails from './HistoryVendorDetails';

const SentDetailsVendor = () => {
    ...
    useEffect(() => {
        // Mock data initialization for the Send Vendor Details Flow
        const initializeVendorDetails = () => {
            const hasInitialized = localStorage.getItem('crest_purchase_vendor_details_initialized');
            ...
            if (!hasInitialized) {
                ...
                const history = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    sku: `SKU${String(800 + i).padStart(3, '0')}`,
                    ...
                    vendorContact: `+91 98765 0000${i}`
                }));

                localStorage.setItem('crest_purchase_pending_vendor_details', JSON.stringify(pending));
                localStorage.setItem('crest_purchase_history_vendor_details', JSON.stringify(history));
                localStorage.setItem('crest_purchase_vendor_details_initialized', 'true');

                setPendingIndents(pending);
                setHistoryIndents(history);
            } else {
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_vendor_details') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_vendor_details') || '[]'));
            }
        };
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_vendor_details', JSON.stringify(updatedPending));
        localStorage.setItem('crest_purchase_history_vendor_details', JSON.stringify(updatedHistory));
    };
    ...
    return (
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-sky-100 shadow-sm">
                <div className="flex-1">
                    <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                        ...
                        Sent Details to Vendor
                    </h1>
                    <p className="text-sm text-slate-500 mt-1">Send Purchase Order details and get vendor confirmation</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="relative shrink-0">
                        ...
                        <FileSignature className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-500" size={14} />
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingVendorDetails indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                ) : (
                    <HistoryVendorDetails indents={filteredHistory} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                )}
            </div>
        </div>
    );
...
export default SentDetailsVendor;
