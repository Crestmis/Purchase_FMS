...
const HistoryVendorDetails = ({ indents, SortDropdown, onSort }) => {
    return (
                        ...
                        <tr>
                            ...
                            >
                                <div className="flex items-center">
                                    PO Status <SortDropdown columnKey="status" />
                                </div>
                            </th>
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                                onClick={() => onSort("vendorContact")}
                            >
                                <div className="flex items-center">
                                    Contact Used <SortDropdown columnKey="vendorContact" />
                                </div>
                            </th>
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                            ...
                            >
                                <div className="flex items-center">
                                    Final Quote (₹) <SortDropdown columnKey="quotedPrice" />
                                </div>
                            </th>
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase
                                        ${item.status === 'Confirmed' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}
                                    `}>
                                        {item.status === 'Confirmed' ? <CheckCircle size={10} /> : <XCircle size={10} />}
                                        {item.status}
                                    </span>
                                </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="bg-slate-50/50 p-2.5 rounded-xl">
                                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Contact Used</label>
                                <span className="text-slate-800 font-bold break-words">{item.vendorContact || '-'}</span>
                            </div>
                            <div className="bg-slate-50/50 p-2.5 rounded-xl">
