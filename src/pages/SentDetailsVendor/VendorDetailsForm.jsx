...
import { X, CheckCircle, Clock, FileSignature, Phone, User, Package, IndianRupee } from 'lucide-react';
import Portal from '../../components/Portal';

const VendorDetailsForm = ({ data, onClose, onSave }) => {
                ...
                <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[460px] overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in slide-in-from-bottom-4 duration-300 border border-white/20">
                    ...
                    <div className="bg-gradient-to-b from-sky-50 to-white border-b border-slate-100 px-5 py-3.5 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-extrabold text-slate-800">PO Distribution</h2>
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{data.serialNo} • Vendor Confirmation</p>
                            </div>
                        </div>
                        <button 
                            onClick={onClose}
                    ...
                    <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-5 space-y-3 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                        ...
                        <div className="bg-sky-50/50 rounded-[1.5rem] p-3.5 border border-sky-100/50">
                            <div className="grid grid-cols-2 gap-y-2.5 gap-x-4">
                                <div className="col-span-2">
                                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Selected Vendor</label>
                                ...
                                <div>
                                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Material</label>
                                ...
                                <div>
                                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Order Value</label>
                                ...
                                <div className="col-span-2 pt-3 border-t border-sky-100">
                                    <span className="bg-sky-100/50 text-sky-700 px-3 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-wide block text-center">
                                        Approved Quantity: {data.reorderQty} {data.unit}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block px-1">Vendor Contact Person / Phone</label>
                        ...
                        <div className="space-y-3">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block px-1">Verification Status</label>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                `}>
                                    <input 
                                        ...
                                        className="sr-only" 
                                    />
                                    <CheckCircle size={20} className={status === 'Confirmed' ? 'text-emerald-500' : 'text-slate-300 group-hover:text-emerald-400'} />
                                    <span className="font-extrabold text-sm uppercase tracking-tight">Confirmed</span>
                                </label>

                                <label className={`
                                    group flex items-center justify-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all active:scale-95
                                    ${status === 'Pending' 
                                        ? 'bg-amber-50 border-amber-500 text-amber-700 ring-4 ring-amber-500/10 shadow-md' 
                                        : 'bg-white border-slate-100 text-slate-500 hover:border-amber-200 hover:bg-amber-50/30'}
                                `}>
                                    <input 
                                        type="radio" 
                                        name="status" 
                                        ...
                                        className="sr-only" 
                                    />
                                    <Clock size={20} className={status === 'Pending' ? 'text-amber-500' : 'text-slate-300 group-hover:text-amber-400'} />
                                    <span className="font-extrabold text-sm uppercase tracking-tight">Wait Reply</span>
                                </label>
                            </div>
                        </div>
                    </form>
                        ...
                        <button 
                            ...
                            className={`flex items-center gap-2 px-8 h-10.5 text-white rounded-2xl text-[10px] font-extrabold uppercase tracking-wider shadow-lg transition-all active:scale-95 hover:-translate-y-0.5
                                ${status === 'Confirmed' 
                                    ...
                                    : 'bg-gradient-to-r from-amber-600 to-amber-500 shadow-amber-500/25 hover:shadow-amber-500/40'}
                            `}
                        >
                            Save Recording
                        </button>
