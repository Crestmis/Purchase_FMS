...
const DocumentReportingAccounts = () => {
    ...
    useEffect(() => {
        ...
        const initializeAccountsSubmission = () => {
            const hasInitialized = localStorage.getItem('crest_purchase_accounts_initialized');
            
            if (!hasInitialized) {
                // Generate 5 pending items (mocking unloaded materials arriving here)
                const pending = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    leadTime: (i % 3) + 2,
                    vehicleNo: `MH 12 AB ${7000 + i}`,
                    arrivalTime: new Date(Date.now() - (i * 3600000)).toISOString(),
                    invoiceNo: `INV-24-${6000 + i}`,
                    unloadedQty: (i + 1) * 40,
                    storageLocation: `Tank ${i + 1}`
                }));
                ...
                const history = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    quotedPrice: (i + 1) * 950,
                    leadTime: (i % 3) + 1,
                    vehicleNo: `MH 12 CD ${8000 + i}`,
                    invoiceNo: `INV-24-${7000 + i}`,
                    unloadedQty: (i + 1) * 30,
                    status: 'Submitted',
                    submissionDate: new Date(Date.now() - (i * 86400000)).toISOString()
                }));

                localStorage.setItem('crest_purchase_pending_accounts', JSON.stringify(pending));
                localStorage.setItem('crest_purchase_history_accounts', JSON.stringify(history));
                localStorage.setItem('crest_purchase_accounts_initialized', 'true');
                
                setPendingIndents(pending);
                setHistoryIndents(history);
            } else {
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_accounts') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_accounts') || '[]'));
            }
        };

        initializeAccountsSubmission();
    }, []);
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_accounts', JSON.stringify(updatedPending));
    ...
    return (
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingAccountsReporting indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                ) : (
                    <HistoryAccountsReporting indents={filteredHistory} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                )}
            </div>
        </div>
    );
...
export default DocumentReportingAccounts;
