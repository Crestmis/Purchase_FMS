...
const HistoryAccountsReporting = ({ indents, SortDropdown, onSort }) => {
    return (
        <div className="flex-1 bg-white/90 backdrop-blur-sm border border-sky-100 rounded-2xl shadow-lg overflow-hidden flex flex-col">
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-sky-50 text-sky-700 font-medium border-b border-sky-100 sticky top-0 z-10">
                        <tr>
                            ...
                            >
                                <div className="flex items-center">
                                    Submission Date <SortDropdown columnKey="submissionDate" />
                                </div>
                            </th>
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                            ...
                            >
                                <div className="flex items-center">
                                    Qty Submitted <SortDropdown columnKey="unloadedQty" />
                                </div>
                            </th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-sky-50">
                        {indents.map((item) => (
                            <tr key={item.id} className="hover:bg-sky-50/50 transition-colors">
                                ...
                                <td className="px-4 py-3 font-medium text-purple-700 uppercase bg-purple-50/50">
                                    {item.invoiceNo || '-'}
                                </td>
                                ...
                                <td className="px-4 py-3 text-slate-600 font-bold">{item.unloadedQty} {item.unit}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="bg-slate-50/50 p-2.5 rounded-xl col-span-2">
                                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Submission Date</label>
                                <span className="text-slate-800 font-medium">{item.submissionDate ? new Date(item.submissionDate).toLocaleString() : '-'}</span>
                            </div>
            ...
            {indents.length === 0 && (
                <div className="text-center py-12 text-slate-400">
                    ...
                    <p>No account submission history found.</p>
                </div>
            )}
        </div>
    );
...
export default HistoryAccountsReporting;
