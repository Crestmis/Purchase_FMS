...
import { X, CheckCircle, XCircle, FileText, IndianRupee, Clock, Truck } from 'lucide-react';
import Portal from '../../components/Portal';

const QuotationForm = ({ data, onClose, onSave }) => {
                ...
                <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[460px] overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in slide-in-from-bottom-4 duration-300 border border-white/20">
                    ...
                    <div className="bg-gradient-to-b from-sky-50 to-white border-b border-slate-100 px-5 py-3.5 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-extrabold text-slate-800">Quotation Entry</h2>
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{data.serialNo} • {data.sku}</p>
                            </div>
                        </div>
                        <button 
                            onClick={onClose}
                    ...
                    <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-5 space-y-3 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                        ...
                        <div className="bg-sky-50/50 rounded-[1.5rem] p-3.5 border border-sky-100/50 flex items-center justify-between">
                            <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5 block">Material</label>
                                <span className="font-bold text-slate-700">{data.name}</span>
                            </div>
                            <div className="text-right">
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5 block">Required Qty</label>
                                <span className="bg-sky-100 text-sky-700 px-3 py-1 rounded-full text-xs font-extrabold">
                                    {data.reorderQty} {data.unit}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block px-1">Vendor / Supplier Name</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block px-1">Quoted Price</label>
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            className="w-full pl-10 pr-4 h-10.5 bg-slate-50/50 border border-slate-200 rounded-2xl outline-none focus:bg-white focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 shadow-sm transition-all text-sm font-medium"
                                            placeholder="0.00"
                                        />
                                    </div>
                                </div>
                                <div>
                                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block px-1">Lead Time</label>
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            className="w-full pl-10 pr-4 h-12 bg-slate-50/50 border border-slate-200 rounded-2xl outline-none focus:bg-white focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 shadow-sm transition-all text-sm font-medium"
                                            placeholder="Days"
                                        />
                                    </div>
                                </div>
                        ...
                        <div className="space-y-3">
                            <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block px-1">Process Action</label>
                            <div className="grid grid-cols-2 gap-4">
                                <label className={`
                                ...
                                `}>
                                    <input 
                                        ...
                                        className="sr-only" 
                                    />
                                    <CheckCircle size={20} className={status === 'Process' ? 'text-emerald-500' : 'text-slate-300 group-hover:text-emerald-400'} />
                                    <span className="font-extrabold text-sm uppercase tracking-tight">Accept</span>
                                </label>
