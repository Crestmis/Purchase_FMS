...
const HistoryQuotation = ({ indents, SortDropdown, onSort }) => {
    return (
                ...
                {indents.map((item) => (
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase
                                        ${item.status === 'Process' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}
                                    `}>
                                        {item.status === 'Process' ? <CheckCircle size={10} /> : <XCircle size={10} />}
                                        {item.status}
                                    </span>
                                </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div className="bg-slate-50/50 p-2.5 rounded-xl">
                                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Quote</label>
