...
import { ClipboardList, Clock, MapPin, Search, Filter, ChevronDown, ArrowUp, ArrowDown } from 'lucide-react';
import PendingGateInReporting from './PendingGateInReporting';
import HistoryGateInReporting from './HistoryGateInReporting';

const GateInReporting = () => {
        ...
        const initializeGateIn = () => {
            ...
            if (!hasInitialized) {
                ...
                const history = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    quotedPrice: (i + 1) * 950,
                    leadTime: (i % 3) + 1,
                    vehicleNo: `MH 12 CD ${4000 + i}`,
                    placementDate: new Date(Date.now() - ((i + 2) * 86400000)).toISOString(),
                    dispatchDate: new Date(Date.now() - ((i + 1) * 86400000)).toISOString(),
                    expectedArrival: new Date(Date.now() - ((i) * 3600000)).toISOString(),
                    status: 'Arrived',
                    arrivalTime: new Date(Date.now() - ((i) * 1800000)).toISOString()
                }));
                ...
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_gatein') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_gatein') || '[]'));
            }
        };
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_gatein', JSON.stringify(updatedPending));
        localStorage.setItem('crest_purchase_history_gatein', JSON.stringify(updatedHistory));
    };
        ...
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-sky-100 shadow-sm">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500 mt-1">Record physical arrival of material vehicles at the facility gate</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="relative shrink-0">
                        ...
                        <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-500" size={14} />
...
export default GateInReporting;
