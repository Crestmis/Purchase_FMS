...
const PendingGateInReporting = ({ indents, onProcess, SortDropdown, onSort }) => {
        ...
        <div className="flex-1 bg-white/90 backdrop-blur-sm border border-sky-100 rounded-2xl shadow-lg overflow-hidden flex flex-col">
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-sky-50 text-sky-700 font-medium border-b border-sky-100 sticky top-0 z-10">
                        <tr>
                            ...
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                                onClick={() => onSort("expectedArrival")}
                            >
                                <div className="flex items-center">
                                    Expected Arrival <SortDropdown columnKey="expectedArrival" />
                                </div>
                            </th>
                            <th 
                                className="px-4 py-3 whitespace-nowrap cursor-pointer hover:bg-sky-100 transition-colors select-none"
                        ...
                        {indents.map((item) => (
                            <tr key={item.id} className="hover:bg-sky-50/50 transition-colors">
                                ...
                                <td className="px-4 py-3 font-medium text-amber-600">
                                    {item.expectedArrival ? new Date(item.expectedArrival).toLocaleDateString() : '-'}
                                </td>
                ...
                {indents.map((item) => (
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                        <div className="flex justify-between items-start border-b border-slate-50 pb-3">
                            ...
                            <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-xl text-xs font-bold">
                                {item.expectedArrival ? new Date(item.expectedArrival).toLocaleDateString() : '-'}
                            </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="bg-slate-50/50 p-2.5 rounded-xl col-span-2">
                                ...
                                <span className="text-slate-800 font-bold uppercase text-lg">{item.vehicleNo || '-'}</span>
                            </div>
                            <div className="bg-slate-50/50 p-2.5 rounded-xl col-span-2">
            ...
            {indents.length === 0 && (
                <div className="text-center py-12 text-slate-400">
                    ...
                    <p>No vehicles pending gate-in log.</p>
                </div>
            )}
...
export default PendingGateInReporting;
