...
const PendingDeduction = ({ indents, onProcess, SortDropdown, onSort }) => {
        ...
        <div className="flex-1 bg-white/90 backdrop-blur-sm border border-sky-100 rounded-2xl shadow-lg overflow-hidden flex flex-col">
                            ...
                            <tr key={item.id} className="hover:bg-sky-50/50 transition-colors">
                                <td className="px-4 py-3 text-center">
                                    <button 
                                        ...
                                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-rose-500 to-rose-400 text-white rounded-lg text-xs font-medium hover:shadow-lg shadow-rose-500/30 transition-all hover:-translate-y-0.5"
                                    >
                                        <Percent size={14} />
                                        Deduct
                                    </button>
                ...
                {indents.map((item) => (
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                        <div className="flex justify-between items-start border-b border-slate-50 pb-3">
                            ...
                            <span className="bg-rose-100 text-rose-700 px-3 py-1 rounded-xl text-xs font-bold uppercase">
                        ...
                        <button 
                            onClick={() => setSelectedIndent(item)}
                            className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-rose-600 to-rose-500 text-white rounded-xl text-xs font-bold shadow-lg shadow-rose-500/20 active:scale-[0.98] transition-all"
                        >
                            <Percent size={16} />
                            Process Quality Deduction
                        </button>
                    </div>
                ))}
            </div>
            ...
            {indents.length === 0 && (
                <div className="text-center py-12 text-slate-400">
                    ...
                    <p>No records pending for quality/shortage deduction.</p>
                </div>
            )}
...
export default PendingDeduction;
