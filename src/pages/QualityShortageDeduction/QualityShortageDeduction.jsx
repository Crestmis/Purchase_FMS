...
import { ClipboardList, Clock, Percent, Search, Filter, ChevronDown, ArrowUp, ArrowDown } from 'lucide-react';
import PendingDeduction from './PendingDeduction';
import HistoryDeduction from './HistoryDeduction';

const QualityShortageDeduction = () => {
    ...
    useEffect(() => {
        // Mock data initialization for the Quality & Shortage Deduction Flow
        const initializeDeductions = () => {
            const hasInitialized = localStorage.getItem('crest_purchase_deductions_initialized');
            
            if (!hasInitialized) {
                // Generate 5 pending items (mocking materials submitted to accounts arriving here)
                const pending = Array.from({ length: 5 }).map((_, i) => ({
                    id: Date.now() + i,
                    serialNo: `SN-P${String(i + 1).padStart(3, '0')}`,
                    sku: `SKU${String(1500 + i).padStart(3, '0')}`,
                    ...
                    leadTime: (i % 3) + 2,
                    vehicleNo: `MH 12 AB ${8000 + i}`,
                    arrivalTime: new Date(Date.now() - (i * 3600000)).toISOString(),
                    invoiceNo: `INV-24-${8000 + i}`,
                    unloadedQty: (i + 1) * 40,
                    submissionDate: new Date(Date.now() - (i * 3600000)).toISOString()
                }));
                ...
                const history = Array.from({ length: 5 }).map((_, i) => ({
                    ...
                    quotedPrice: (i + 1) * 950,
                    leadTime: (i % 3) + 1,
                    vehicleNo: `MH 12 CD ${9000 + i}`,
                    invoiceNo: `INV-24-${9000 + i}`,
                    unloadedQty: (i + 1) * 30,
                    shortageQty: (i % 3 === 0) ? 2 : 0,
                    deductionAmount: (i % 3 === 0) ? 2500 : 0,
                    remittanceStatus: 'Debit Note Issued'
                }));

                localStorage.setItem('crest_purchase_pending_deductions', JSON.stringify(pending));
                localStorage.setItem('crest_purchase_history_deductions', JSON.stringify(history));
                localStorage.setItem('crest_purchase_deductions_initialized', 'true');
                
                setPendingIndents(pending);
                setHistoryIndents(history);
            } else {
                setPendingIndents(JSON.parse(localStorage.getItem('crest_purchase_pending_deductions') || '[]'));
                setHistoryIndents(JSON.parse(localStorage.getItem('crest_purchase_history_deductions') || '[]'));
            }
        };
    ...
    const processIndent = (processedData) => {
        ...
        localStorage.setItem('crest_purchase_pending_deductions', JSON.stringify(updatedPending));
        localStorage.setItem('crest_purchase_history_deductions', JSON.stringify(updatedHistory));
    };
    ...
    return (
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-sky-100 shadow-sm">
                <div className="flex-1">
                    <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                        ...
                        Quality & Shortage Deduction
                    </h1>
                    <p className="text-sm text-slate-500 mt-1">Issue Debit Notes for material shortages or quality non-compliance</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="relative shrink-0">
                        ...
                        <Percent className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-500" size={14} />
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingDeduction indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                ) : (
                    <HistoryDeduction indents={filteredHistory} SortDropdown={SortDropdown} onSort={toggleSort} sortConfig={sortConfig} />
                )}
            </div>
        </div>
    );
...
export default QualityShortageDeduction;
