    ...
    return (
        <div className="h-full flex flex-col p-4 lg:p-6 space-y-4">
            ...
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                {activeTab === 'pending' ? (
                    <PendingIntimation indents={filteredPending} onProcess={processIndent} SortDropdown={SortDropdown} />
                ) : (
                    <HistoryIntimation indents={filteredHistory} SortDropdown={SortDropdown} />
                )}
            </div>
        </div>
    );
