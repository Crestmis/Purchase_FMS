...
const HistoryIntimation = ({ indents }) => {
    return (
            ...
            <div className="hidden lg:block overflow-x-auto flex-1">
                <table className="w-full text-sm text-left">
                    <thead className="bg-sky-50 text-sky-700 font-medium border-b border-sky-100 sticky top-0 z-10">
                        <tr>
                            <th className="px-4 py-3 whitespace-nowrap">Msg Status</th>
                            <th className="px-4 py-3 whitespace-nowrap">Group Name</th>
                        ...
                        {indents.map((item) => (
                            <tr key={item.id} className="hover:bg-sky-50/50 transition-colors">
                                <td className="px-4 py-3">
                                    ...
                                    `}>
                                        {item.status === 'Sent' ? <CheckCircle size={12} /> : <XCircle size={12} />}
                                        {item.status}
                                    </span>
                                </td>
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-sky-100 shadow-sm p-4 space-y-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase
                                        ${item.status === 'Sent' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}
                                    `}>
                                        {item.status === 'Sent' ? <CheckCircle size={10} /> : <XCircle size={10} />}
                                        {item.status}
                                    </span>
                                </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="bg-slate-50/50 p-2.5 rounded-xl">
                                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">WhatsApp Group</label>
                                <span className="text-slate-800 font-bold break-words">{item.groupName || '-'}</span>
                            </div>
                            <div className="bg-slate-50/50 p-2.5 rounded-xl">
...
export default HistoryIntimation;
